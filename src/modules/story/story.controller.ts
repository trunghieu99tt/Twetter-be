...
export class StoryController {
    ...
    async createStory(@GetUser() user: UserDocument, @Body() createStoryDto: StoryDTO): Promise<ResponseDTO> {
    ...
    @Get('')
    @ApiQueryGetMany()
    async getStories(@QueryGet() query: QueryPostOption): Promise<ResponseDTO> {
        const stories = await this.storyService.getStories(query.options);
    ...
    @Patch('/:id')
    @UseGuards(MyTokenAuthGuard)
    async updateStory(@GetUser() user: UserDocument, @Param('id') id: string): Promise<ResponseDTO> {
    ...
    @Delete('/:id')
    @UseGuards(MyTokenAuthGuard)
    async deleteStory(@GetUser() user: UserDocument, @Param('id') id: string): Promise<ResponseDTO> {
