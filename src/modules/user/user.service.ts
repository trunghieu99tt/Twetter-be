...
export class UserService {
  ...
  async updateUserById(
    ...
    if (['admin', 'root-admin'].includes(requestUser.role)) {
      throw new BadRequestException(UserService.name, 'You are not admin');
    }
    ...
    if (
      ...
      user.following.splice(
        user.following.findIndex(
          (user) => user.id.toString() === userToFollow._id.toString(),
        ),
        1,
      );
  ...
  async getMostActiveUsers() {
    ...
    const usersTweetCounter = await Promise.all(
      users.map(async (user: UserDocument) => {
        const userTweet = await this.tweetService.countTweetByUser(user._id);
        return {
          user,
